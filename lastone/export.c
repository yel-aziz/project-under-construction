...
/*   Updated: 2022/06/22 04:51:19 by ysakine          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		while (command[i])
		{
			...
			if (len_before_char(command[i],'=') != ft_strlen(command[i]))
			{
				...
				else
				{
					...
					var->part_one = ft_calloc(len_before_char(command[i],'=') + 1, sizeof(char));
					var->part_two = ft_calloc(ft_strlen(command[i]) - len_before_char(command[i],'=') 
										+ 1, sizeof(char));
					ft_strlcpy(var->part_one, command[i], len_before_char(command[i], '=') + 1);
					ft_strlcpy(var->part_two, &command[i][len_before_char(command[i], '=') + 1], 
					ft_strlen(command[i]) - len_before_char(command[i], '='));
